"""
plc_runtime.py — Pipeline PLC Runtime
MORBION SCADA v02
...
log = logging.getLogger("plc_runtime.pipeline")
...
class PLCRuntime:
    ...
    def upload_program(self, source: str) -> bool:
        try:
            ...
            self._last_error = f"Parse error: {e}"
            log.error("Program upload rejected: %s", e)
            return False
        except Exception as e:
            self._last_error = str(e)
    ...
    def scan(self, state, dt: float = 0.1) -> None:
        ...
        Reads inputs from ProcessState into input image.
        Executes ST program against input image.
        Writes output image back to ProcessState.
        Never raises.
        """
        with self._lock:
        ...
        # Build input image from ProcessState
        variables = {}
        with state:
        ...
        # Write output image back to ProcessState
        with state:
