...
class PLCWorker(QThread):
    ...
    def run(self):
        try:
            if self.action == "sync":
                p = self.rest.plc_get_program(self.process)
                v = self.rest.plc_get_variables(self.process)
                if p: self.finished.emit({"prog": p, "vars": v})
                else: self.feedback.emit("Sync Failed", theme.RED)
            elif self.action == "upload":
                res = self.rest.plc_upload(self.process, self.source)
                col = theme.GREEN if res.get("ok") else theme.RED
                self.feedback.emit("UPLOAD OK" if res.get("ok") else f"FAILED: {res.get('error')}", col)
            elif self.action == "reload":
                res = self.rest.plc_reload(self.process)
                col = theme.GREEN if res.get("ok") else theme.RED
                self.feedback.emit("RELOADED" if res.get("ok") else "RELOAD FAILED", col)
...
class PLCView(QWidget):
    ...
    def __init__(self, rest):
        super().__init__()
        self._rest = rest; self._process = "ps"; self._build_ui(); self._refresh()
    ...
    def _on_sync_done(self, data):
        ...
        # DRILL INTO NESTED STRUCTURE: variables -> inputs/outputs
        v_data = data['vars'].get("variables", {}) if data['vars'] else {}
        for k in v_data.get("inputs", {}): self._in_list.addItem(k)
        for k in v_data.get("outputs", {}): self._out_list.addItem(k)
