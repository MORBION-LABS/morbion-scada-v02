...
class PipelineView(BaseProcessView):
    ...
    def _build_data_panel(self) -> QWidget:
        ...
        loc = QLabel("Kenya Pipeline Co. — Petroleum Transfer  |  Port 508")
        loc.setStyleSheet(theme.STYLE_DIM)
        layout.addWidget(loc)

        press_box = QGroupBox("PRESSURES & FLOW")
        press_layout = QVBoxLayout(press_box)
        self._inlet_press  = GaugeWidget("Inlet Pressure",  "bar", 0, 10,  lo_alarm=1.0)
        self._outlet_press = GaugeWidget("Outlet Pressure", "bar", 0, 60,  hi_alarm=55, lo_alarm=30)
        self._flow_rate    = GaugeWidget("Flow Rate",       "m³/hr",0, 600, lo_alarm=200)
        ...
        for w in [self._duty_running, self._duty_speed, self._duty_current,
                  self._duty_power, self._duty_diff]:
            duty_layout.addWidget(w)
        ...
        self._spark_outlet = SparklineWidget("Outlet Pressure", "bar",   hi_alarm=55, lo_alarm=30)
        self._spark_flow   = SparklineWidget("Flow Rate",        "m³/hr", lo_alarm=200)
    ...
    def _build_control_panel(self) -> QWidget:
        ...
        start_btn.clicked.connect(lambda: threading.Thread(
            target=lambda: self._rest.write_register("pipeline", 5, 1), daemon=True).start())
        stop_btn.clicked.connect(lambda: threading.Thread(
            target=lambda: self._rest.write_register("pipeline", 5, 0), daemon=True).start())
        duty_layout.addWidget(start_btn)
        duty_layout.addWidget(stop_btn)
        duty_layout.addWidget(RegisterWriteRow(
            "Speed (RPM)", self._rest, "pipeline", 3, scale=1, hint="0-1480"))
        layout.addWidget(duty_box)
        ...
        sb_start.clicked.connect(lambda: threading.Thread(
            target=lambda: self._rest.write_register("pipeline", 7, 1), daemon=True).start())
        sb_stop.clicked.connect(lambda: threading.Thread(
            target=lambda: self._rest.write_register("pipeline", 7, 0), daemon=True).start())
        ...
        writes_layout.addWidget(RegisterWriteRow(
            "Outlet Valve (%)",      self._rest, "pipeline", 9,  scale=10,  hint="0-100"))
        writes_layout.addWidget(RegisterWriteRow(
            "Inject Outlet P (bar)", self._rest, "pipeline", 1,  scale=100, hint=">55=overpressure"))
        writes_layout.addWidget(RegisterWriteRow(
            "Inject Leak Flag",      self._rest, "pipeline", 13, scale=1,   hint="1=inject leak"))
    ...
    def update_data(self, data: dict):
        ...
        sb_run = data.get("standby_pump_running", False)
        self._standby_running.set_value(
            "RUNNING" if sb_run else "STANDBY",
            override_color=theme.AMBER if sb_run else theme.TEXT_DIM)
