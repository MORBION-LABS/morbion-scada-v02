"""
alarms_view.py — Alarm management view
MORBION SCADA v02
"""
...
class AlarmsView(QWidget):
    ...
    def update_data(self, alarms: list):
        ...
        for alarm in alarms:
            ...
            for col, text in enumerate(cells):
                ...
                item.setForeground(
                    QColor(theme.TEXT_DIM) if acked else color)
                if acked:
                    item.setText(f"[ACK] {text}" if col == 0 else text)
                self._table.setItem(row, col, item)

    def _ack_all(self):
        operator = self._config.get("operator", "OPERATOR")
        threading.Thread(
            target=self._do_ack,
            args=("all", operator),
            daemon=True,
    ...
    def _ack_selected(self):
        ...
        alarm_id = self._table.item(row, 1)
        if not alarm_id:
            return
        operator = self._config.get("operator", "OPERATOR")
        threading.Thread(
            target=self._do_ack,
            args=(alarm_id.text(), operator),
            daemon=True,
        ).start()
