...
class _ProcessCard(QGroupBox):
    ...
    def add_gauge(self, g): self._layout.addWidget(g)
    def add_value(self, v): self._layout.addWidget(v)

    def set_online(self, online: bool, fault: int = 0, fault_text: str = ""):
        if not online:       self._badge.set_offline()
        elif fault > 0:      self._badge.set_fault(fault, fault_text)
...
class OverviewView(QWidget):
    ...
    def _build_ui(self):
        ...
        self._ps_card  = _ProcessCard("PUMPING STATION",  "Nairobi Water — Municipal")
        self._ps_level = GaugeWidget("Tank Level", "%", 0, 100, hi_alarm=90, lo_alarm=10)
        self._ps_flow  = GaugeWidget("Pump Flow",  "m³/hr", 0, 150)
        self._ps_press = ValueLabel("Discharge Pressure", "bar", hi_alarm=8.0)
        ...
        self._hx_card   = _ProcessCard("HEAT EXCHANGER",  "KenGen Olkaria — Geothermal")
        self._hx_eff    = GaugeWidget("Efficiency", "%", 0, 100, lo_alarm=45)
        self._hx_t_hot  = ValueLabel("T Hot In",    "°C", hi_alarm=200)
        self._hx_t_cold = ValueLabel("T Cold Out",  "°C", hi_alarm=95)
        ...
        self._bl_card  = _ProcessCard("BOILER",           "EABL/Bidco — Industrial Steam")
        self._bl_press = GaugeWidget("Drum Pressure", "bar", 0, 12, hi_alarm=10, lo_alarm=6)
        self._bl_level = GaugeWidget("Drum Level",    "%",   0, 100, hi_alarm=80, lo_alarm=20)
        self._bl_steam = ValueLabel("Steam Flow", "kg/hr")
        ...
        self._pl_card  = _ProcessCard("PIPELINE",         "Kenya Pipeline Co. — Petroleum")
        self._pl_out   = GaugeWidget("Outlet Pressure", "bar", 0, 60, hi_alarm=55, lo_alarm=30)
        self._pl_flow  = GaugeWidget("Flow Rate",        "m³/hr", 0, 600)
    ...
    def update_data(self, data: dict):
        ...
        self._ps_card.set_online(ps.get("online", False), ps.get("fault_code", 0), ps.get("fault_text", ""))
        ...
        self._hx_card.set_online(hx.get("online", False), hx.get("fault_code", 0), hx.get("fault_text", ""))
        ...
        self._bl_card.set_online(bl.get("online", False), bl.get("fault_code", 0), bl.get("fault_text", ""))
        ...
        self._pl_card.set_online(pl.get("online", False), pl.get("fault_code", 0), pl.get("fault_text", ""))
