"""
boiler_view.py — Boiler detailed view
MORBION SCADA v02
"""
...
class BoilerView(BaseProcessView):
    ...
    def _build_data_panel(self) -> QWidget:
        ...
        burner_layout.addWidget(self._burner_state)
        burner_layout.addWidget(self._fuel_flow)
        burner_layout.addWidget(self._q_burner)
        burner_layout.addWidget(self._flue_temp)
        burner_layout.addWidget(self._comb_eff)
        ...
        self._spark_press = SparklineWidget(
            "Drum Pressure", "bar", hi_alarm=10, lo_alarm=6)
        self._spark_level = SparklineWidget(
    ...
    def _build_control_panel(self) -> QWidget:
        ...
        def set_burner(v):
            threading.Thread(
                target=lambda: self._rest.write_register("boiler", 6, v),
                daemon=True
            ).start()

        off_btn.clicked.connect(lambda: set_burner(0))
        low_btn.clicked.connect(lambda: set_burner(1))
        high_btn.clicked.connect(lambda: set_burner(2))
        ...
        writes_layout.addWidget(RegisterWriteRow(
            "FW Pump Speed (RPM)", self._rest,
            "boiler", 7, scale=1, hint="0-1450"))
        writes_layout.addWidget(RegisterWriteRow(
            "Steam Valve (%)", self._rest,
            "boiler", 8, scale=10, hint="0-100"))
        writes_layout.addWidget(RegisterWriteRow(
            "FW Valve (%)", self._rest,
            "boiler", 9, scale=10, hint="0-100"))
        writes_layout.addWidget(RegisterWriteRow(
            "Blowdown Valve (%)", self._rest,
            "boiler", 10, scale=10, hint="0-100"))
        ...
        inject_layout.addWidget(RegisterWriteRow(
            "Drum Pressure (bar)", self._rest,
            "boiler", 0, scale=100, hint=">10 triggers overpressure"))
        inject_layout.addWidget(RegisterWriteRow(
            "Drum Level (%)", self._rest,
            "boiler", 2, scale=10, hint="<20 triggers low water"))
